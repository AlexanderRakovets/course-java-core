...
class Task08 {
    ...
    static String generateTotalPriceList(int startNumberItems, double startPriceAllItems, int differentialNumberItems, double differentialSell, int sizeTotalPrice) {
        double productСost = startPriceAllItems / startNumberItems;
        double totalSell = 0.0;
        final var DEFINITION_PERCENTAGES = 100;
        var currentNumberItems = startNumberItems;
        String totalPriceList = startNumberItems + " - " + startPriceAllItems + " with sell " + totalSell + "%\n";
        for (var startingPointPriceComparison = 1; startingPointPriceComparison < sizeTotalPrice; startingPointPriceComparison++) {
            currentNumberItems  += differentialNumberItems;
            totalSell +=  differentialSell;
            startPriceAllItems = productСost * currentNumberItems;
            double priceWithSell = NumberUtil.roundValueToTwoDigitsForMantissa(startPriceAllItems - startPriceAllItems * totalSell / DEFINITION_PERCENTAGES);
            totalPriceList += currentNumberItems + " - " + priceWithSell + " with sell " + totalSell + "%";
            if (startingPointPriceComparison < sizeTotalPrice - 1) {
                totalPriceList += "\n";
            }
        }
